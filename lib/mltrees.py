...
def test_feature_continuous(D, feature, feature_info):
    ...
    for split in range(feature_info[feature]["min"], feature_info[feature]["max"] + 1):
        ...
        impurity = (len(left) / len(D) * array_purity(left) + len(right) / len(D) * array_purity(right))
...
def select_feature(D, features, feature_info):
    impurities = [test_feature(D, feature, feature_info) for feature in features]
...
def gen_model(D, default_label, max_depth=float("inf"), feature_sample_size=-1, id=-1):
    ...
    if feature_sample_size > 0:
        features = random.sample(list(columns), int(feature_sample_size * len(columns)))
    else:
    ...
    tree = construct_tree(D, list(feature_info.keys()), feature_info, default_label, max_depth=max_depth, inc_depth=inc_depth)
...
def test_model(D, testD, model, evauluate_func=tree_predict):
    ...
    print("Test Data False Positives:", test_false_positives, (test_false_positives / len(testD)) * 100, "%")
    print("Test Data False Negatives:", test_false_negatives, (test_false_negatives / len(testD)) * 100, "%")
